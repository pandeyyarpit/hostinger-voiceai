...
def _get_slots_calcom(date_str: str) -> list:
    creds = get_cal_creds()
    try:
        # Cal.com v2 uses /v2/slots with params 'start' and 'end' (not startTime/endTime)
        resp = requests.get(
            f"{CAL_V2_BASE}/slots",
            headers={
...
async def _create_booking_calcom(
    ...
    email = caller_email.strip() if caller_email and caller_email.strip() else f"{caller_phone.replace('+','').replace(' ','')}@voiceagent.placeholder"
    logger.info(f"[CAL] Booking with email={email}, start={start_time}")
    payload = {
        "eventTypeId": creds["event_id"],
        "start": start_time,
        "attendee": {
